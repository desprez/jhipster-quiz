...
/**
 * Performance test for the Quizz entity.
 */
public class QuizzGatlingTest extends Simulation {
            ...
            exec(http("Get all quizzes").get("/api/quizzes").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new quizz")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_quizz_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created quizz").get("${new_quizz_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created quizz").delete("${new_quizz_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Quizz entity").exec(scn);
