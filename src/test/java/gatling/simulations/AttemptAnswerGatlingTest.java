...
/**
 * Performance test for the AttemptAnswer entity.
 */
public class AttemptAnswerGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all attemptAnswers").get("/api/attempt-answers").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new attemptAnswer")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_attemptAnswer_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created attemptAnswer").get("${new_attemptAnswer_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created attemptAnswer").delete("${new_attemptAnswer_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AttemptAnswer entity").exec(scn);
