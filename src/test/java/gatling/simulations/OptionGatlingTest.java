 ...
 */
public class OptionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all options").get("/api/options").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new option")
                        .post("/api/options")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"statement\": \"SAMPLE_TEXT\"" + ", \"index\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_option_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created option").get("${new_option_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created option").delete("${new_option_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Option entity").exec(scn);
